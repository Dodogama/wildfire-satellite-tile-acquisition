...
def send_to_bucket(**context):
    ...
    for row in batch:
        ...
        wrs_row = wrscoords[3:]
        year = date[:4]
        content = f"collection{coll}/level-2/standard/oli-tirs/{year}/{wrs_path}/{wrs_row}/{fileID[:-3]}/{fileID}.TIF"

        filename = f"processed_files/{fileID}.txt"
        ...
        tiles_sql = f"""
            INSERT INTO Inventory.tiles 
            (fileID, bucketLink, wrsRow, wrsPath, date)
            VALUES ('{fileID}', '{bucket_link}', '{wrs_row}',
            '{wrs_path}', '{date_formatted}');
        """
        hook.run(tiles_sql)
        ...
        request_sql = f"""
            UPDATE Inventory.RequestFiles
            SET downloadTime = {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
            WHERE fileID = '{fileID}';
        """
        hook.run(request_sql)


# this was our code to pull data from Landsat AWS Bucket
def write_to_gcs(**context):
    ...
    key = 'collection02/level-2/standard/oli-tirs/2013/023/037/LC08_L2SP_023037_20130531_20200912_02_T1/LC08_L2SP_023037_20130531_20200912_02_T1_SR_B1.TIF'
    ...
    print(f"Loaded Landsat band: {file_like}")

    os.makedirs('/tmp/airflow_downloaded', exist_ok=True)
    with open('/tmp/airflow_downloaded/test_af.TIF', 'wb') as f:
        f.write(file_bytes)
    print("File downloaded successfully.")

    with open('/tmp/airflow_downloaded/test_af.TIF', 'rb') as f:
        file_content = f.read()
        print(f"File content: {file_content[:100].decode('utf-8')}")


# DAG definition
with DAG(
    dag_id='aws_configure_test_v2',
    ...
    load_task = PythonOperator(
        task_id='load_aws_task',
        python_callable=write_to_gcs,
        provide_context=True
    )

    # fetch_task >> process_task >> mark_task >> send_task
    load_task
