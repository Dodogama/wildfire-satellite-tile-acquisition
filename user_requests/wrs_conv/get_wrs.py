""" Converts latitude/longitude coordinates to Landsat WRS-2 path/row using
USGS shapefiles.

Adapted by Zac Spalding from the original code
(https://github.com/robintw/LatLongToWRS/blob/master/README.md)
by Robert Wilson
"""
...
class ConvertToWRS:
    ...
    * Landsat WRS-2 Path/Row Shapefiles - download from USGS site
     (http://landsat.usgs.gov/tools_wrs-2_shapefile.php), you want
     wrs2_descending.zip
    ...
    """
    def __init__(self, shapefile=pathlib.Path(__file__).parent.resolve() /
                 "WRS2_descending.shp"):
